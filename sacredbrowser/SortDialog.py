...
class SortDialog(QtGui.QDialog):
    ...
    def __init__(self):
        ...
        # the list is automatically shortened to a maximum of MaxSortKeyCount
        # entries
        self.currentSortOrder = []

        # container for the search widgets. each lines consists of a field
        # label and the search keys
        self.fieldLines = []
        ...
        self.mainLayout.addWidget(
            self.stayOnTopButton, 0, 0, QtCore.Qt.AlignBottom)
        ...
        self.mainLayout.addWidget(
            self.closeButton, 1, 0, QtCore.Qt.AlignBottom)
    ...
    def reset(self, fieldList):
        ...
        for fieldX in range(len(self.currentSortOrder) - 1, -1, -1):
            if self.currentSortOrder[fieldX] not in fieldList:
                # that's why we count backwards
                del self.currentSortOrder[fieldX]
        ...
        self.currentSortOrder = self.currentSortOrder[
            0:SortDialog.MaxSortKeyCount]  # OK even for shorter lists
#         print('NOW ORDER:: ',self.currentSortOrder)
        ...
        for pos in range(len(self.fieldList)):
            ...
            theseButtons = [SortButton(str(keypos + 1), pos, keypos)
                            for keypos in range(currentFieldCount)]

            # connect
            for keypos in range(currentFieldCount):
                theseButtons[keypos].sortClicked.connect(
                    self.slotSortButtonClicked)

            self.mainLayout.addWidget(thisLabel, pos, 0)
            for keypos in range(currentFieldCount):
                self.mainLayout.addWidget(
                    theseButtons[keypos], pos, keypos + 1)

            self.fieldLines.append([thisLabel] + theseButtons)
        self.mainLayout.addWidget(self.stayOnTopButton, len(
            self.fieldList), 0, 1, currentFieldCount + 1, QtCore.Qt.AlignBottom)
        self.mainLayout.addWidget(self.closeButton, len(
            self.fieldList) + 1, 0, 1, currentFieldCount + 1, QtCore.Qt.AlignBottom)
    ...
    def updateButtons(self):
        ...
        for row in range(len(self.fieldList)):
            for col in range(currentFieldCount):
                ...
                self.fieldLines[row][
                    col + 1].setChecked(self.currentSortOrder[col] == thisField)
    ...
    def slotSortButtonClicked(self, row, col):
        # col is 0 .. MaxSortKeyCount - 1, thisField is the field whose sorting
        # is to be changed
