...
'''
    ...
    def validateQuery(self, queryText):
        ...
        def parseOrCondition(fieldName, content):
            ...
            if matchOb is None:
                raise FilterChoice.WrongQueryException(
                    'Illegal 'or' command (required format [ ... ]) in line %d' % lX)
            ...
            if len(subContentData) == 1:
                ...
                # make 'or' query
                resultDict = {'$or': [{fieldName: possiblyConvert(
                    val.strip())} for val in subContentData]}
        ...
        def parseRegexp(fieldName, content):
            ...
            if matchOb is None:
                raise FilterChoice.WrongQueryException(
                    'Illegal regexp (required format / ... /) in line %d' % lX)
        ...
        for (lX, line) in enumerate(lines):
            ...
            try:
                ...
                raise FilterChoice.WrongQueryException(
                    'Line %d must contain exactly one colon (:), contains %d' % (lX, line.count(':')))

            fieldName = fieldName.strip()
            content = content.strip()

            # interpret context, several cases ('or' condition, regexp, normal
            # text)
            fieldName = str('config.' + fieldName)
            if fieldName in processedResultFieldNames:
                raise FilterChoice.WrongQueryException(
                    'Key %s specified twice in line %d' % (fieldName, lX))
    ...
    def reset(self, forceEmpty=False):
        ...
        if not forceEmpty:
            if self.application.currentDatabase is not None and self.application.currentRunCollection is not None:
                lastQuery = self.application.settings.value(
                    'Collections' + '/' + self.application.collectionSettingsName + '/' + 'query')
        ...
        try:
            ...
            if self.application.currentDatabase is not None and self.application.currentRunCollection is not None:
                self.application.settings.setValue(
                    'Collections' + '/' + self.application.collectionSettingsName + '/' + 'query', self.editor.toPlainText())
